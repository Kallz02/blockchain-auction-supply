...
function createShipment(address _reciever,uint256 _pickupTime , uint256 _distance , uint256 _price ) public payable{
  ...
  require(msg.value==_price,"Payment Should Match the price");
  ...
  typeShipments.push(
    TypeShipment(
      msg.sender,_reciever,_pickupTime,0,_distance,_price,ShipmentStatus.PENDING,false
  )
  );
...
require(shipment.receiver==_reciever,"Invalid Reciever");
require(shipment.status==ShipmentStatus.PENDING,"Shipment aldready in transit");
...
require(!shipment.isPaid,"Shipment aldready paid");
...
(address,address,uint256,uint256,uint256,uint256,ShipmentStatus,bool){
  ...
  return(
    shipment.sender,shipment.receiver, shipment.pickupTime,shipment.deliveryTime,shipment.distance,shipment.price,shipment.status,shipment.isPaid
  );
}
